...
def phase_correction(
    bpm_data_orig: pd.DataFrame, lin_frame: pd.DataFrame, plane: str
) -> pd.DataFrame:
    """
    ...
    phase_range = np.outer(tune, int_range - bpm_data.shape[1] / 2) + np.outer(
        phase, np.ones(bpm_data.shape[1])
    )
