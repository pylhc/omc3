...
Accelerator-Class for the ``AD`` machine.
...
class Ad(Accelerator):
    """Parent Class for AD-types."""
    NAME = "ad"
    RE_DICT = {AccElementTypes.BPMS: r".*DR.U*",
               AccElementTypes.MAGNETS: r".*",
               AccElementTypes.ARC_BPMS: r".*DR.U*"
               }

    # Public Methods ##########################################################
    @staticmethod
    def get_parameters():
        params = super(Ad, Ad).get_parameters()
    ...
    def get_file(self, filename: str) -> Path:
        """Get filepaths for AD files."""
        ad_dir = CURRENT_DIR / self.NAME
        ad_year_dir = self.get_dir()

        for dir_ in (ad_year_dir, ad_dir):
