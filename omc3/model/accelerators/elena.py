...
Accelerator-Class for the ``ELENA`` machine.
...
class Elena(Accelerator):
    """Parent Class for ELENA-types."""
    NAME = "elena"
    ...
    @staticmethod
    def get_parameters():
        params = super(Elena, Elena).get_parameters()
    ...
    def get_file(self, filename: str) -> Path:
        """Get filepaths for ELENA files."""
        elena_dir = CURRENT_DIR / self.NAME
        elena_year_dir = self.get_dir()

        for dir_ in (elena_year_dir, elena_dir):
...
class _ElenaSegmentMixin(object):
