...
Accelerator-Class for the ``LEIR`` machine.
...
class Leir(Accelerator):
    """Parent Class for LEIR-types."""
    NAME = "leir"
    RE_DICT = {AccElementTypes.BPMS: r".*UE.*",
               AccElementTypes.MAGNETS: r".*",
               AccElementTypes.ARC_BPMS: r".*UE.*"
               }

    # Public Methods ##########################################################
    @staticmethod
    def get_parameters():
        params = super(Leir, Leir).get_parameters()
        params.add_parameter(
            name="year",
            type=int,
            default=2022,
            choices=(2018, 2021, 2022),
    ...
    def get_file(self, filename: str) -> Path:
        """Get filepaths for LEIR files."""
        leir_dir = CURRENT_DIR / self.NAME
        leir_year_dir = self.get_dir()

        for dir_ in (leir_year_dir, leir_dir):
...
class _LeirSegmentMixin(object):
