...
Accelerator-Class for the ``SPS`` machine.
...
class Sps(Accelerator):
    """Parent Class for SPS-types."""
    NAME = "sps"
    RE_DICT = {AccElementTypes.BPMS: r"BP.*",
    ...
    @staticmethod
    def get_parameters():
        params = super(Sps, Sps).get_parameters()
    ...
    def get_file(self, filename: str) -> Path:
        """Get filepaths for SPS files."""
        sps_dir = CURRENT_DIR / self.NAME
        sps_year_dir = self.get_dir()

        for dir_ in (sps_year_dir, sps_dir):
