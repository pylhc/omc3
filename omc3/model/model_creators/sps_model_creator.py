...
class SpsModelCreator(ModelCreator, ABC):
    ...
    def get_base_madx_script(self) -> str:
        ...
        seq_path = self._madx_path(accel.acc_model_path / "sps.seq")
        ...
        if accel.modifiers is not None:  # includes the strengths file
            for modifier in accel.modifiers:
                modifier_path = self._madx_path(modifier)
                madx_script += f"call, file = '{modifier_path}'; {MODIFIER_TAG}\n"

        toolkit_path = self._madx_path(accel.acc_model_path / "toolkit" / "macro.madx")
        madx_script += (
    ...
    def get_madx_script(self) -> str:
        ...
        twiss_path = self._madx_path(self.output_dir / TWISS_DAT)
        twiss_elements_path = self._madx_path(self.output_dir / TWISS_ELEMENTS_DAT)
        madx_script += (
        ...
        if use_excitation or accel.drv_tunes is not None:
            # allow user to modify script and enable excitation, if driven tunes are given
            twiss_ac_path = self._madx_path(self.output_dir / TWISS_AC_DAT)
            madx_script += (
                f"if(use_acd == 1){{\n"
