...
def calculate(measure_input: DotDict, input_files: InputFiles, invariants: dict[str, tfs.TfsDataFrame], header: dict[str, Any]):
    ...
    for crdt in CRDTS:
        ...
        result_df[AMPLITUDE], result_df[f"{ERR}{AMPLITUDE}"] = get_crdt_amplitude(
            crdt, invariants, amplitudes, err_amplitudes
        )

        phases = (
            phase_sign * input_files.get_data(result_df, lines_and_phases[PHASE])
        ...
        err_phases = np.sqrt(
            input_files.get_data(result_df, lines_and_phases[f"{ERR}{PHASE}"]) ** 2
        ...
        result_df[REAL] = (
            np.cos(PI2 * result_df[PHASE].to_numpy()) * result_df[AMPLITUDE].to_numpy()
        )
        result_df[IMAG] = (
...
def get_crdt_invariant(crdt, invariants):
    ...
    err_crdt_invariant = np.sqrt(
        (exp["X"] * crdt_invariant / invariants["X"].T[0]) ** 2 * invariants["X"].T[1] ** 2
...
def get_crdt_amplitude(crdt, invariants, line_amps, line_amp_errors):
    ...
    for idx in range(line_amps.shape[0]):
        amps[idx], err_amps[idx] = fit_amplitude(
            line_amps[idx], line_amp_errors[idx], crdt_invariants, err_crdt_invariants
        )
