...
def get_params():
    ...
    params.add_parameter(
        help="BBQ data as data frame or tfs file.", name="input", required=True,
    )
    params.add_parameter(
        help="Kick file as data frame or tfs file.", name="kick",
    )
    params.add_parameter(
        help="Save figure to this location.", name="output", type=str,
    )
    params.add_parameter(help="Show plot.", name="show", action="store_true")
    params.add_parameter(
        help="X-Axis limits. (yyyy-mm-dd HH:mm:ss.mmm)", name="x_lim", type=float, nargs=2,
    )
    params.add_parameter(
        help="Y-Axis limits.", name="y_lim", type=float, nargs=2,
    )
    params.add_parameter(
        help="x_axis interval that was used in calculations.", name="interval", type=float, nargs=2,
    )
    params.add_parameter(
        help="Plot two axis into the figure.", name="two_plots", action="store_true",
    )
    return params
...
def main(opt):
    ...
    _save_options(opt)
    bbq_df: tfs.TfsDataFrame = kick_mod.read_timed_dataframe(opt.input) if isinstance(
        opt.input, str
    ) else opt.input
    opt.pop("input")
    ...
    if opt.kick:
        if opt.interval:
            raise ValueError(
                "Interval and kick-file given. Both are used for the same purpose. "
                "Please only use one."
            )
        ...
        for p in PLANES:
            with suppress(KeyError):
                window: dict = bbq_df.headers[get_mav_window_header(p)]
...
def _plot_bbq_data(
    bbq_df: Union[tfs.TfsDataFrame, pd.DataFrame],
    interval=None,
    ...
    Args:
        bbq_df (Union[tfs.TfsDataFrame, pd.DataFrame]0: BBQ Dataframe with moving average columns.
        interval: start and end time of used interval, will be marked with red bars.
        x_lim (Tuple[float, float]): x limits (time).
        y_lim (Tuple[float, float]): y limits (tune).
        two_plots (bool): Plots each tune in its own axes if `True`.
    ...
    pstyle.set_style(
        "standard",
        {u"figure.figsize": [12.24, 7.68], u"lines.marker": u"", u"lines.linestyle": u""},
    )
    ...
    for idx, plane in enumerate(PLANES):
        ...
        handles[idx] = axs[idx].plot(
            ...
            color=pcolors.change_color_brightness(color, 0.4),
            marker="o",
        ...
        handles[len(PLANES) + idx] = axs[idx].plot(
            ...
            color=pcolors.change_color_brightness(color, 0.7),
            marker=".",
...
def _save_options(opt):
    if opt.output:
        ...
        save_options_to_config(
            str(out_path / formats.get_config_filename(__file__)), OrderedDict(sorted(opt.items()))
        )
