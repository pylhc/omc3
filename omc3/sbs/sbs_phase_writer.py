...
def _propagate_error_phase(errb0, erra0, dphi, bet0, alf0):
    ...
    -(1/2.*alf0/bet0))*errb0)**2+((-(1/2.*np.cos(4*np.pi*dphi))+(1/2.))*erra0)**2)/(2*np.pi)
...
def create_phase_segment(folder_measure, output_dir, label):
	...
	for plane in planes:
		...
		twiss_forward      = tfs.read(Path(f"{output_dir}/twiss_{label}.dat"), index="NAME") #model_propagation = propagated_models.propagation
		twiss_forward_corr = tfs.read(Path(f"{output_dir}/twiss_{label}_cor.dat"), index="NAME") #model_back_propagation = propagated_models.back_propagation
		twiss_back         = tfs.read(Path(f"{output_dir}/twiss_{label}_back.dat"), index="NAME") #model_cor = propagated_models.corrected
		twiss_back_corr    = tfs.read(Path(f"{output_dir}/twiss_{label}_cor_back.dat"), index="NAME") #model_back_cor = propagated_models.corrected_back_propagation
		...
		pd_out[NAME_DICT["ERRPROPPHASE"]] = _propagate_error_phase(err_betx_start, err_alfx_start, twiss_forward_ph[NAME_DICT["MU"]], betx_start, alfx_start)
		pd_out[NAME_DICT["ERRPROPPHASE"]] = np.sqrt(pd_out[NAME_DICT["ERRPROPPHASE"]]**2+pd_out[NAME_DICT["STDERRPHASE"]]**2)

		pd_out[NAME_DICT["ERRCORPHASE"]] = _propagate_error_phase(err_betx_start, err_alfx_start, twiss_forward_corr_ph[NAME_DICT["MU"]], betx_start, alfx_start)
		pd_out[NAME_DICT["ERRCORPHASE"]] = np.sqrt(pd_out[NAME_DICT["ERRCORPHASE"]]**2+pd_out[NAME_DICT["ERRPROPPHASE"]]**2)

		pd_out[NAME_DICT["ERRBACKPHASE"]] = _propagate_error_phase(err_betx_end, err_alfx_end, twiss_back_ph[NAME_DICT["MU"]], betx_end, alfx_end)
		pd_out[NAME_DICT["ERRBACKPHASE"]] = np.sqrt(pd_out[NAME_DICT["ERRBACKPHASE"]]**2+pd_out[NAME_DICT["STDERRPHASE"]]**2)

		pd_out[NAME_DICT["ERRBACKCORPHASE"]] = _propagate_error_phase(err_betx_end, err_alfx_end, twiss_back_corr_ph[NAME_DICT["MU"]], betx_end, alfx_end)
		pd_out[NAME_DICT["ERRBACKCORPHASE"]] = np.sqrt(pd_out[NAME_DICT["ERRBACKPHASE"]]**2+pd_out[NAME_DICT["ERRBACKCORPHASE"]]**2)
		
		tfs.write(Path(f"{output_dir}/sbsphase{plane}_{label}.out"), pd_out, save_index="NAME")		
