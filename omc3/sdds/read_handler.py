...
import numpy as np
from sdds.classes import (SddsFile, Include, Column, Parameter,
                          Definition, Array, Data, Description,
                          ENCODING, ENCODING_LEN,
                          NUMTYPES, NUMTYPES_CAST, NUMTYPES_SIZES)
...
def _read_header(inbytes: IO[bytes]) ->\
    ...
    assert version == "SDDS1",\
        "This module is compatible with SDDS v1 only... "\
        "are there really other versions?"
...
def _sort_definitions(orig_defs: List[Definition]) -> List[Definition]:
    ...
    According to the specification parameters appear first in data pages then
    arrays and then columns. Inside each group they follow the order of
    appearance in the header.
    """
    definitions = [definition for definition in orig_defs
                   if isinstance(definition, Parameter)]
    definitions.extend([definition for definition in orig_defs
...
def _read_data(data: Data,
               definitions: List[Definition],
               inbytes: IO[bytes]) -> List[Any]:
    if data.mode == "binary":
        return _read_data_binary(definitions, inbytes)
    if data.mode == "ascii":
        # TODO: ascii mode not supported yet
        raise NotImplementedError
    raise ValueError(f"Unknown data mode {data.mode}.")
...
def _read_data_binary(definitions: List[Definition],
                      inbytes: IO[bytes]) -> List[Any]:
...
def _read_bin_param(inbytes: IO[bytes], definition: Parameter) ->\
        Union[int, float, str]:
    try:
...
def _read_bin_array(inbytes: IO[bytes], definition: Array) -> Any:
    ...
    if definition.type == "string":
        ...
        return str_array
    return _read_bin_numeric(inbytes,
                             definition.type,
                             total_len).reshape(dims)


def _read_bin_array_len(inbytes: IO[bytes], num_dims: int) ->\
        Tuple[List[int], int]:
