...
def _write_header(sdds_file: SddsFile, outbytes: IO[bytes]) -> List[str]:
    ...
    if sdds_file.description is not None:
        outbytes.write(_sdds_def_as_str(sdds_file.description)
                       .encode(ENCODING))
...
def _write_data(names: List[str], sdds_file: SddsFile, outbytes: IO[bytes])\
        -> None:
    # row_count:
...
def _write_parameters(param_gen: Iterable[Parameter], outbytes: IO[bytes]):
    for param_def, value in param_gen:
        if param_def.type == "string":
            ...
            outbytes.write(
                np.array(value, dtype=NUMTYPES[param_def.type]).tobytes()
            )


def _write_arrays(array_gen: Iterable[Array], outbytes: IO[bytes]):
    for array_def, value in array_gen:
        outbytes.write(np.array(len(value),
                                dtype=NUMTYPES["long"]).tobytes())
        if array_def.type == "string":
            for string in value:
                _write_string(string, outbytes)
        else:
            outbytes.write(
                np.array(value, dtype=NUMTYPES[array_def.type]).tobytes()
            )
