...
def delete_content_of_dir(path_to_dir: str):
    """
    Deletes all folders, files and symbolic links in given directory.

    Args:
        path_to_dir (str): directory to delete the contents of.
    """
...
def deleteFilesWithoutGitignore(pathToDirectory) -> bool:
    """
    Deletes all files in the given ``pathToDirectory`` except of the file with the name
    **.gitignore**.

    Returns:
        ``True`` if the directory exists and the files were deleted, otherwise ``False``.
    """
...
def replace_keywords_in_textfile(path_to_textfile, dict_for_replacing, new_output_path=None):
    """
    This function replaces all keywords in a textfile with the corresponding values in the provided
    dictionary.
