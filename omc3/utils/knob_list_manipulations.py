...
def get_vars_by_classes(classes_or_knobs: Iterable[str] | None, all_knobs_by_class: dict[str, list[str]]) -> list[str]:
    ...
    add_classes = {c for c in classes_or_knobs if c in all_knobs_by_class}

    remove_classes = {c[1:] for c in classes_or_knobs if c[0] == "-" and c[1:] in all_knobs_by_class}
    if remove_classes:
        LOGGER.info(f"The following classes will be removed from the correctors:\n{remove_classes!s}")

    add_knobs = {knob for knob in classes_or_knobs if knob[0] != "-" and knob not in all_knobs_by_class}
    if add_knobs:
    ...
    remove_knobs = {knob[1:] for knob in classes_or_knobs if knob[0] == "-" and knob[1:] not in all_knobs_by_class}
    if remove_knobs:
