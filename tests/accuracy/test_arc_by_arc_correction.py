...
def get_mqt_params(beam):
    return CorrectionParameters(
        twiss=CORRECTION_INPUTS / f"inj_beam{beam}" / f"twiss_mqt_quadrupole_error.dat",
        correction_filename=CORRECTION_TEST_INPUTS / f"changeparameters_injb{beam}_mqt_quadrupole.madx",
        optics_params=[f"{PHASE}X", f"{PHASE}Y", TUNE],
        weights=[1., 1., 0.8],
        variables=["MQT_ONLY_NON_ATS"],
...
def test_lhc_arc_by_arc_global_correct(tmp_path, model_inj_beams, orientation):
    ...
    correction_params = get_mqt_params(beam)
    iterations = 1   # '3' tests a single correction + one iteration, as the last (3rd) correction is not tested itself.
