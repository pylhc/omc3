...
def test_freekick_harpy(_test_file, _model_file):
    ...
    for plane in PLANES:
        ...
        assert (
            _rms(
                _diff(
                    lin[plane].loc[:, f"TUNE{plane}"].to_numpy(),
                    model.loc[:, f"TUNE{plane}"].to_numpy(),
                )
            )
            < LIMITS["F1"]
        )
        ...
        assert (
            _rms(
                _rel_diff(
                    ...
                    model.loc[:, f"AMP{plane}"].to_numpy(),
                )
            )
            < LIMITS["A1"]
        )
        ...
        assert (
            _rms(
                _angle_diff(
                    lin[plane].loc[:, f"MU{plane}"].to_numpy(),
                    model.loc[:, f"MU{plane}"].to_numpy(),
                )
            )
            < LIMITS["P1"]
        )
...
def test_harpy_3d(_test_file, _model_file):
    ...
    assert (
        _rms(
            _rel_diff(
                lin["X"].loc[:, f"AMPZ"].to_numpy() * lin["X"].loc[:, f"AMPX"].to_numpy() * 2,
                AMPZ * BASEAMP,
            )
        )
        < LIMITS["A2"]
...
def _assert_spectra(lin, model):
    for plane in PLANES:
        ...
        assert (
            _rms(
                _diff(
                    lin[plane].loc[:, f"FREQ{_couple(plane)}"].to_numpy(),
                    model.loc[:, f"TUNE{_other(plane)}"].to_numpy(),
                )
            )
            < LIMITS["F2"]
        )
        ...
        assert (
            _rms(
                _rel_diff(
                    lin[plane].loc[:, f"AMP{_couple(plane)}"].to_numpy()
                    * lin[plane].loc[:, f"AMP{plane}"].to_numpy()
                    * 2,
                    COUPLING * model.loc[:, f"AMP{_other(plane)}"].to_numpy(),
                )
            )
            < LIMITS["A2"]
        )
        ...
        assert (
            _rms(
                _angle_diff(
                    lin[plane].loc[:, f"PHASE{_couple(plane)}"].to_numpy(),
                    model.loc[:, f"MU{_other(plane)}"].to_numpy(),
                )
            )
            < LIMITS["P2"]
        )
