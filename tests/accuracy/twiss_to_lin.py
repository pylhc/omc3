...
def generate_lin_files(model, tune, nattune, motion="_d", dpp=0.0, beam_direction=1):
    ...
    for plane in PLANES:
        ...
        lin["CO"] = dpp * model.loc[:, f"D{plane}{motion}"] + np.random.randn(nbpms) * (
        ...
        lin[f"NATTUNE{plane}"] = nattune[plane] + (
        ...
        ) + noise_freq_domain * np.random.randn(nbpms)
        ...
        lin[f"NATAMP{plane}"] = NAT_OVER_DRV * np.sqrt(
            ACTION * model.loc[:, f"BET{plane}{MOTION['free']}"]
        ) + noise_freq_domain * np.random.randn(nbpms)

        lin[f"PHASE{COUP[plane]}"] = (
            np.remainder(
        ...
        lin[f"AMP{COUP[plane]}"] = COUPLING * np.sqrt(
            ACTION
