...
OMC3_DIR: Path = Path(omc3.__file__).parent
...
@contextmanager
def mock_module_import(module: str, replacement):
...
def model_inj_beams(request, tmp_path_factory: TempPathFactory) -> dict:
...
def acc_models_lhc_2025(tmp_path_factory: TempPathFactory) -> Path:
    return clone_acc_models(tmp_path_factory, "lhc", 2025)


@pytest.fixture(scope="session")
def acc_models_lhc_2022(tmp_path_factory: TempPathFactory) -> Path:
    return clone_acc_models(tmp_path_factory, "lhc", 2022)


@pytest.fixture(scope="session")
def acc_models_lhc_2018(tmp_path_factory: TempPathFactory) -> Path:
    return clone_acc_models(tmp_path_factory, "lhc", 2018)


@pytest.fixture(scope="session")
def acc_models_psb_2021(tmp_path_factory: TempPathFactory) -> Path:
    return clone_acc_models(tmp_path_factory, "psb", 2021)


@pytest.fixture(scope="session")
def acc_models_sps_2025(tmp_path_factory: TempPathFactory) -> Path:
    return clone_acc_models(tmp_path_factory, "sps", 2025)


@pytest.fixture(scope="session")
def acc_models_ps_2021(tmp_path_factory: TempPathFactory) -> Path:
    return clone_acc_models(tmp_path_factory, "ps", 2021)


def clone_acc_models(tmp_path_factory: TempPathFactory, accel: str, year: int) -> Path:
