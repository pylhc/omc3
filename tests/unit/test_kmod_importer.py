...
from tests.unit.test_kmod_averaging import (
    _get_all_tfs_filenames as _get_averaged_filenames,
    _get_measurement_dir,
    _get_reference_dir,
)
from tests.unit.test_kmod_import import _get_model_path, _get_bpm_referece_path, _get_betastar_referece_path 
...
def test_full_kmod_import_beam(tmp_path, beam):
    ...
    # Run the import ---
    import_kmod_results(
        meas_paths=[_get_measurement_dir(ip=ip, i_meas=i) for ip in (1, 5) for i in range(1, 3)],
        beam=beam,
    ...
    assert len(list(average_dir.glob("*.pdf"))) == 4  # beta-star and waist per IP  
    ...
    for ip in (1, 5):
        for out_name in _get_averaged_filenames(ip, beta=beta):
            ...
            ref_file = tfs.read(_get_reference_dir(ip, n_files=2) / out_name)
