...
PS_MODEL = join(
    dirname(__file__), pardir, pardir, "omc3", "model", "accelerators", "ps", "2018", "strength"
)
...
def _create_input_lhc():
    ...
    iotools.write_string_into_new_file(
        join(BASE_OUTPUT, MODIFIERS_MADX), f"call, file='{join(BASE_OUTPUT, 'strengths.madx')}';\n"
    )
...
def _create_input_ps():
    ...
    iotools.copy_item(
        join(PS_MODEL, "PS_LE_LHC_low_chroma.str"), join(BASE_OUTPUT, "strengths.madx")
    )
    iotools.write_string_into_new_file(
